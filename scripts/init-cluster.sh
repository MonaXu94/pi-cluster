...
ssh -t $SSH_USER@$MASTER_IP "sudo curl -sfL https://get.k3s.io | K3S_TOKEN=SECRET sh -s - server --cluster-init"
...
for MASTER in "${MASTER_NODES[@]:1}"; do
  ...
  ssh -t $SSH_USER@$MASTER "sudo curl -sfL https://get.k3s.io | K3S_TOKEN=SECRET sh -s - server --server https://$MASTER_IP:6443"
done
...
for WORKER in "${WORKER_NODES[@]}"; do
  ...
  ssh -t $SSH_USER@$WORKER "sudo curl -sfL https://get.k3s.io | K3S_TOKEN=SECRET sh -s - agent --server https://$MASTER_IP:6443"
done
